...
export const DetailView = React.memo(
  ({ onClose, steadyStateId, isEmbedded = false }: DetailViewProps) => {
    const { steadyStateId: routeSteadyStateId } = useParams<{ steadyStateId: string }>();
    const actualSteadyStateId = steadyStateId || routeSteadyStateId;
    // 添加一个缓存标志，防止重复加载同一数据
    ...
    useEffect(() => {
      ...
      if (dataLoadedRef.current || !actualSteadyStateId) {
        if (!actualSteadyStateId) {
          setError('未找到稳态区间ID');
          setLoading(false);
        }
        return;
      ...
      const loadData = async () => {
        try {
          setLoading(true);
          setError(null);
          console.log('开始加载数据，稳态区间ID:', actualSteadyStateId);
          ...
          if (cachedRawData) {
            ...
            for (let i = 0; i < totalChunks; i++) {
              ...
              const hasTargetData = chunkData.some(
                (d: any) => d.稳态区间编号 && +d.稳态区间编号 === +actualSteadyStateId
              );
          ...
          const filteredData = csvData
            .filter((d: any) => d.稳态区间编号 && +d.稳态区间编号 === +actualSteadyStateId)
            .map((d: any) => {
          ...
          if (filteredData.length === 0) {
            throw new Error(`未找到稳态区间 ${actualSteadyStateId} 的有效数据`);
          }
